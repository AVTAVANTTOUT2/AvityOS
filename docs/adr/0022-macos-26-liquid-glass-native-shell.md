...
3. `DesignSystem.swift` holds the shared primitives — metrics, the `GlassCard`
   panel, the `ConnectionBadge`, the `PairingStep` and the diagnostic row — so
   the header, Settings and the menu bar cannot describe the same state
   differently.
4. Transport state is a single `ConnectionTone` token carrying tint, SF Symbol
   and label together. Colour is never the only signal.
5. The main window hides its title bar and the embedded UI runs edge to edge
   behind a glass header that reserves room for the window controls.
...
7. The menu-bar companion uses `menuBarExtraStyle(.window)` so it can report
   live transport state and counts rather than list inert menu items.
